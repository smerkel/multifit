...
;
;
; Old routines to create Fit2d macro for a single 2D diffraction image
; This macro forces fit2d to perform "cake" multiple times for the same image
; It is quite slow.
;
;
; *******************************************************************
...
;
;
; Old routines to create Fit2d macro for multiple 2D diffraction images
; This macro forces fit2d to perform "cake" multiple times for the same image
; It is quite slow.
;
;
...
;
;
; New subroutines to create Fit2d macro for multiple images
; For each image, fit2d will generate one cake only.
;
;
; *******************************************************************


FUNCTION makefit2dmac2DData, log, stash
...
PRO setFit2dmac2DDataFromPrevious, file, stash
...
PRO fit2dmac2DData_event, ev
common files, extension, datadirectory, outputdirectory, defaultdirectory, jcpdsdirectory
WIDGET_CONTROL, ev.TOP, GET_UVALUE=stash
WIDGET_CONTROL, ev.ID, GET_UVALUE=uval
log=stash.log
if (uval eq 'OK') then begin
    result = makefit2dmac2DData(log,stash)
    if (FIX(result) ne 1) then begin
        logit, log, "Macro creation failed"
        tmp = DIALOG_MESSAGE(result, /ERROR)
    endif else begin
	...
	if (oldmacro ne '') then setFit2dmac2DDataFromPrevious, oldmacro, stash
...
PRO fit2dmac2DData, leader, log
common fonts, titlefont, boldfont, mainfont, avFontHeight
common files, extension, datadirectory, outputdirectory, defaultdirectory, jcpdsdirectory
common experiment, wavelength, detectordistance, experimenttype
fit2d = WIDGET_BASE(/COLUMN, Title='Create macro for fit2d (Regular 2D data, multipe images)', GROUP_LEADER=leader, TAB_MODE=1)
; Options
